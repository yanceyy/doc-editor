...
import type { ElementType } from "editor-core";
import { debouncedSaveToLocalStorage } from "shared";
...
const data =
    jsonString === null
        ? initialData
        : (JSON.parse(jsonString) as ElementType[]);
...
editor.observe("update", (context: BoardCanvas) => {
    debouncedSaveToLocalStorageInstance("data", JSON.stringify(context.data));
});

editor.observe("input", (context: BoardCanvas) => {
    debouncedSaveToLocalStorageInstance("data", JSON.stringify(context.data));
});
